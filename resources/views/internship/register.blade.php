...
.reg-hero h1 { font-family:'Outfit',sans-serif;font-weight:800;font-size:clamp(2.2rem,4vw,3rem);color:var(--text-main); margin-bottom:15px; letter-spacing:-0.02em; }
...
.form-label { font-size:0.9rem;color:var(--text-muted);font-weight:600;margin-bottom:8px; }
.form-control-int {
    background: rgba(255,255,255,0.03); border:1px solid rgba(255,255,255,0.1);
    border-radius:14px; color:var(--text-main); padding:14px 18px; width:100%;
    transition: all 0.3s; font-size: 1rem;
}
body.light-mode .form-control-int { background: #fff; border-color: #cbd5e1; }
...
<section class="reg-hero">
    <div class="container">
        ...
        <p style="color:var(--text-muted)">Your payment is verified. Set a password to access your dashboard.</p>
    </div>
</section>
        ...
        <div class="reg-card" data-aos="zoom-in">
            ...
            <form action="{{ route('internship.register.store', $token) }}" method="POST">
                ...
                <div class="mb-4 text-center">
                    ...
                    <h5 style="font-weight:700;color:var(--text-main);margin:0">{{ $account->application->full_name }}</h5>
                    <div style="color:var(--text-muted);font-size:.85rem">{{ $account->application->email }}</div>
                </div>

...
<div class="tech-card">
    <form action="{{ route('admin.services.store') }}" method="POST">
        ...
        <div class="row g-4">
            <div class="col-md-8">
                <div class="mb-4">
                    <label for="title" class="v3-form-label">PROTOCOL TITLE</label>
                    <input type="text" class="v3-form-control" id="title" name="title" value="{{ old('title') }}" placeholder="e.g. Cloud Architecture Design" required>
                </div>

                <div class="mb-4">
                    <label for="icon_class" class="v3-form-label">ICON SIGNATURE (Font Awesome)</label>
                    <input type="text" class="v3-form-control" id="icon_class" name="icon_class" value="{{ old('icon_class') }}" placeholder="fas fa-code-branch">
                </div>
                
                <div class="mb-4">
                    <label for="short_description" class="v3-form-label">BRIEFING (SHORT DESC)</label>
                    <textarea class="v3-form-control" id="short_description" name="short_description" rows="3" placeholder="Truncated technical summary for listing...">{{ old('short_description') }}</textarea>
                </div>

                 <div class="mb-4">
                    <label for="full_description" class="v3-form-label">CORE LOGIC (FULL DESCRIPTION)</label>
                    <textarea class="v3-form-control" id="full_description" name="full_description" rows="6" placeholder="Initialize detailed technical documentation (HTML support enabled)...">{{ old('full_description') }}</textarea>
                </div>
            </div>
            ...
            <div class="col-md-4">
                ...
                <div class="mb-4">
                    ...
                    <input type="number" class="v3-form-control" id="rating" name="rating" value="{{ old('rating', 5) }}" step="0.1" min="0" max="5">
                </div>
            </div>
        </div>

                    ...
                    <div class="col-md-6">
                        ...
                        <select class="v2-form-control" id="client_id" name="client_id">
                            ...
                            @foreach($clients as $client)
                                <option value="{{ $client->id }}" {{ (isset($selectedClientId) && $selectedClientId == $client->id) || old('client_id') == $client->id ? 'selected' : '' }}>
                                    {{ $client->name }} ({{ $client->company_name ?? 'Individual' }})
                                </option>
                            @endforeach
                        </select>
                        <small class="text-v2-muted mt-2 d-block">Linking to a client enables unified CRM tracking.</small>
                    </div>
                    <div class="col-md-6">
                        <label for="client_name" class="v2-form-label">PUBLIC CLIENT NAME</label>
                        <input type="text" class="v2-form-control" id="client_name" name="client_name" value="{{ old('client_name') }}" placeholder="Organization name for public display">
                    </div>
                    <div class="col-md-6">

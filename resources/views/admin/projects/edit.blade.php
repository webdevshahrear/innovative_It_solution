...
<div class="tech-card">
    <form action="{{ route('admin.projects.update', $project) }}" method="POST" enctype="multipart/form-data">
        ...
        <div class="row g-4">
            <div class="col-md-8">
                <div class="mb-4">
                    <label for="title" class="v3-form-label">PROJECT IDENTITY</label>
                    <input type="text" class="v3-form-control" id="title" name="title" value="{{ old('title', $project->title) }}" placeholder="e.g. Cyber Security Dashboard" required>
                </div>
                
                <div class="mb-4">
                    <label for="description" class="v3-form-label">INTEL / DESCRIPTION</label>
                    <textarea class="v3-form-control" id="description" name="description" rows="6" placeholder="Project technical summary and highlights...">{{ old('description', $project->description) }}</textarea>
                </div>

                <div class="row g-4">
                    <div class="col-md-6">
                        <label for="client_name" class="v3-form-label">ENTITY / CLIENT</label>
                        <input type="text" class="v3-form-control" id="client_name" name="client_name" value="{{ old('client_name', $project->client_name) }}" placeholder="Client organization name">
                    </div>
                    <div class="col-md-6">
                        <label for="project_url" class="v3-form-label">LIVE SECTOR URL</label>
                        <input type="url" class="v3-form-control" id="project_url" name="project_url" value="{{ old('project_url', $project->project_url) }}" placeholder="https://project-link.com">
                    </div>
                    <div class="col-md-6">
                        <label for="display_order" class="v3-form-label">SEQUENCE ORDER</label>
                        <input type="number" class="v3-form-control" id="display_order" name="display_order" value="{{ old('display_order', $project->display_order) }}">
                    </div>
                </div>
            </div>
            ...
            <div class="col-md-4">
                ...
                <div class="mb-4">
                    ...
                    @if($project->desktop_image)
                        <div class="asset-preview-v3 mb-3">
                            <img src="{{ filter_var($project->desktop_image, FILTER_VALIDATE_URL) ? $project->desktop_image : asset('uploads/projects/'.$project->desktop_image) }}" alt="Preview">
                        </div>
                    @endif
                ...
                <div class="mb-4">
                    ...
                    @if($project->mobile_image)
                        <div class="asset-preview-v3 mobile mb-3">
                            <img src="{{ filter_var($project->mobile_image, FILTER_VALIDATE_URL) ? $project->mobile_image : asset('uploads/projects/'.$project->mobile_image) }}" alt="Mobile Preview">
                        </div>
                    @endif
                    <div class="v3-file-upload">
...
<style>
    ...
    .asset-preview-v3 img { width: 100%; height: 120px; object-fit: cover; }
    .asset-preview-v3.mobile img { width: 100px; height: 150px; display: block; margin: 0 auto; }

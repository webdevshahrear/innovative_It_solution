...
<div class="page-header d-flex justify-content-between align-items-center mb-4">
    ...
    <a href="{{ route('admin.inquiries.index') }}" class="btn-tech-outline">
        <i class="fas fa-satellite-dish me-2"></i> Signal List
    </a>
</div>
...
<div class="row g-4">
    <div class="col-lg-8">
        <div class="tech-card h-100">
            <div class="d-flex align-items-center justify-content-between mb-4 pb-3 border-bottom border-v3">
            ...
            <div class="mb-4">
                <label class="v3-form-label">SIGNAL SUBJECT</label>
                <div class="v3-static-info fw-bold">{{ $inquiry->subject }}</div>
            </div>

            <div class="mb-4">
                <label class="v3-form-label">DATA PAYLOAD (MESSAGE)</label>
                <div class="v3-message-box">
                    {{ $inquiry->message }}
                </div>
            </div>
    ...
    <div class="col-lg-4">
        <div class="tech-card">
            ...
            <div class="d-flex align-items-center mb-5">
                <div class="operative-avatar-v3 text-uppercase d-flex align-items-center justify-content-center fw-bold fs-4 text-v3-accent" style="background: rgba(99, 102, 241, 0.1);">
                    {{ substr($inquiry->name, 0, 1) }}
                </div>
                <div class="ms-3">
                    <div class="fw-bold text-white fs-5">{{ $inquiry->name }}</div>
                    <div class="text-v3-muted small">External Operative</div>
                </div>
            </div>
            
            <div class="space-y-4">
                <div class="mb-4 pb-3 border-bottom border-v3">
                    <label class="v3-form-label">ELECTRONIC UPLINK (EMAIL)</label>
                    <a href="mailto:{{ $inquiry->email }}" class="text-decoration-none fw-bold text-v3-accent d-flex align-items-center">
                        <i class="far fa-envelope me-2"></i> {{ $inquiry->email }}
                    </a>
                </div>
                <div class="mb-4 pb-3 border-bottom border-v3">
                    <label class="v3-form-label">TEMPORAL COORDINATES</label>
                    <div class="text-white fw-bold d-flex align-items-center">
                        <i class="far fa-calendar-alt me-2 text-v3-muted"></i> {{ $inquiry->created_at->format('M d, Y') }}
                    </div>
                    <div class="text-v3-muted small ps-4">{{ $inquiry->created_at->format('H:i A') }} ({{ $inquiry->created_at->diffForHumans() }})</div>
                </div>
                <div class="mb-0">
                    <label class="v3-form-label">DESTRUCTIVE ACTIONS</label>
                    <form action="{{ route('admin.inquiries.destroy', $inquiry->id) }}" method="POST" onsubmit="return confirm('Purge signal history?')">
                        @csrf
                        @method('DELETE')
                        <button type="submit" class="btn btn-outline-danger w-100 border-v3 py-2 text-danger opacity-75 hover-opacity-100">
...
<style>
    .border-v3 { border-color: var(--v3-border) !important; }
    ...
    .v3-static-info { background: rgba(255,255,255,0.03); border: 1px solid var(--v3-border); border-radius: 10px; padding: 0.75rem 1rem; color: white; }
    .v3-message-box { background: rgba(255,255,255,0.03); border: 1px solid var(--v3-border); border-radius: 12px; padding: 1.5rem; color: #e2e8f0; line-height: 1.8; white-space: pre-wrap; font-size: 0.95rem; }
    
    .operative-avatar-v3 { width: 56px; height: 56px; border-radius: 50%; border: 2px solid var(--v3-border); box-shadow: 0 0 15px rgba(99, 102, 241, 0.1); }
    ...
    .btn-tech-outline { display: inline-flex; padding: 0.6rem 1.2rem; background: transparent; border: 1px solid var(--v3-border); color: var(--v3-text-muted); text-decoration: none; border-radius: 10px; font-weight: 700; font-size: 0.8rem; transition: 0.3s; cursor: pointer; }
    .btn-tech-outline:hover { border-color: white; color: white; background: rgba(255,255,255,0.05); }
    .text-v3-muted { color: rgba(255,255,255,0.4); }

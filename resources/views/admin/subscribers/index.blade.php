                ...
                <tr>
                    <td>
                        <div class="fw-bold text-white d-flex align-items-center">
                            <i class="fas fa-paper-plane me-3 text-v3-accent opacity-50"></i>
                            {{ $subscriber->email }}
                        </div>
                    </td>
                    <td><span class="text-v3-muted">{{ $subscriber->created_at->format('M d, Y') }}</span></td>
                    <td>
                    ...
                    <td class="text-end">
                        <div class="d-flex justify-content-end gap-2">
                            <form action="{{ route('admin.subscribers.update', $subscriber->id) }}" method="POST">
                                ...
                                <button type="submit" class="action-btn-v3 edit" title="Toggle Authorization">
                            ...
                            <form action="{{ route('admin.subscribers.destroy', $subscriber->id) }}" method="POST" class="d-inline" onsubmit="return confirm('Purge subscriber record?');">
                                @csrf
                                @method('DELETE')
                                <button type="submit" class="action-btn-v3 delete" title="Terminal Purge">
...
<style>
    ...
    .action-btn-v3 { width: 36px; height: 36px; border-radius: 10px; display: flex; align-items: center; justify-content: center; border: 1px solid var(--v3-border); background: rgba(255, 255, 255, 0.03); color: var(--v3-text-muted); transition: all 0.3s; text-decoration: none !important; cursor: pointer; }
